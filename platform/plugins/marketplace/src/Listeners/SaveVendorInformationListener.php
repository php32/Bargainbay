...
class SaveVendorInformationListener
{
     ...
     */
    public function __construct(StoreInterface $storeRepository, VendorInfoInterface $vendorInfoRepository, Request $request)
    {
    ...
    {
        $customer = $event->user;
        if (get_class($customer) == Customer::class && !$customer->is_vendor && $this->request->input('is_vendor') == 1) {
                ...
                if ($mailer->templateEnabled('verify_vendor')) {
                    EmailHandler::setModule(MARKETPLACE_MODULE_SCREEN_NAME)
                        ->setVariableValues([
                            ...
                            'store_link'       => route('marketplace.unverified-vendors.edit', $customer->id),
                        ]);
