...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function processPostCheckoutOrder($products, $request, $token, $sessionCheckoutData, $response)
    {
        ...
        $wallet_enable = session('wallet_enable');
        ...
        $mpSessionData = Arr::get($sessionCheckoutData, 'marketplace', []);
        $walletSessionData = Arr::get($sessionCheckoutData, 'wallet_enable', []);
    ...
    public function handleCheckoutOrderByStore(
        ...
        if ($request->has('wallet_amount') && $request->wallet_amount == 1) {
            ...
            $request->merge([
                'use_wallet' => $request->wallet_amount,
    ...
    public function processPaymentMethodPostCheckout($request, $totalAmount)
    {
        ...
        if ($request->has('use_wallet') && $request->use_wallet == 1) {
            if ($request->wallet_amount >= $paymentData['amount']) {
                $paymentData['type'] = PaymentMethodEnum::WALLET;
            }
            $paymentData['wallet_amount'] = $request->wallet_amount;
    ...
    {
        $transaction = auth('customer')->user()->deposit($wallet_amount); 
        return $transaction;
    }
    ...
    public function processShippingDiscountOrderData($products, $token, $sessionCheckoutData, $request)
    {
        ...
        $wallet_enable = 0;
        if ($request->has('wallet_use')) {
            $wallet_enable = $request->wallet_use;
            
            OrderHelper::setOrderSessionData($token, ['wallet_enable' => $wallet_enable]);
            if ($wallet_enable == 1) {
                session()->put('wallet_enable', $wallet_enable);
            } else if($wallet_enable == 0) {
                session()->forget('wallet_enable');
            }
        }

        if (session()->has('wallet_enable')) {
            $wallet_enable = 1;
        }
    ...
    public function afterOrderStatusCompleted(OrderModel $order)
    {
        ...
        if ($order->store && $order->store->customer) {
            ...
            if ($vendorInfo->id) {
                ...
                $currentBalance = $customer->vendor_balance;
                ...
                try {
                    ...
                    $vendorInfo->vendor_balance += $amountByCurrency;
