...
class VendorInfo extends BaseModel
{
    ...
    public function getSignatureKey($isNew = false, $item = null)
    {
        ...
        $balance      = $isNew ? $item->vendor_balance : ($item->getOriginal('vendor_balance') ?: $item->vendor_balance);
    ...
    protected static function booted()
    {
        static::creating(function (&$vendorInfo) {
            ...
            $vendorInfo->vendor_balance = $vendorInfo->vendor_balance ?: 0;
        ...
        static::updating(function (&$vendorInfo) {
            if ($vendorInfo->id) {
                $balanceOriginal      = $vendorInfo->getOriginal('vendor_balance');
                $balance              = $vendorInfo->vendor_balance;
