                    ...
                    <div id="cart-item" class="position-relative">
                        ...
                        <div class="mt-2 p-2">
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 float-right">
                                    @php
                                        $totalPrice = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? format_price(0) : format_price(Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount);

                                        if ($wallet_enable == 1) {
                                            $totalPrice = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? 0 : (Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount);

                                            $total_wallet_amount = auth('customer')->user()->balance;
                                            if ($totalPrice > $total_wallet_amount) {
                                                $totalPrice = $totalPrice - $total_wallet_amount;
                                            }
                        ...
                        <form action="{{ route('payments.checkout') }}" method="post">
                            ...
                            <div class="form-group">
                                <div class="custome-checkbox">
                                    ...
                                    @else
                                    <label for="is_default" class="form-check-label">
                                        <span>Wallet amount ({{ format_price(auth('customer')->user()->balance) }})</span>
                                    </label>
                                        <a href="{{ route('customer.wallet') }}">{{ __('Add wallet amount') }}</a>
                                    @endif
                                </div>   
                            </div>
                            @endif
                            ...
                            <div class="position-relative">
                                ...
                                if($wallet_enable == 1){
                                    ...
                                    $priceCheck = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? 0 : (Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount);

                                    $stripeChecked = (auth('customer')->user()->balance > $priceCheck) ? 0 : 1;
                                } else {
                                    $totalPrice1 = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? 0 : format_price(Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount, null, true);
